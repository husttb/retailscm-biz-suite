...
零售供应链中台基础系统，前端和后端加起来一共大约130万行代码，管理界面通过javascript高阶函数定制，Java后台主要通过增加方法或者重写已经存在的大量方法来灵活定制。
...
目前的模型还处于比较初级的阶段，后面升级围绕以下几个方面进行
* SaaS和点对点化，目前除了核心平台商家功能完整以外，上下游供应商和核心商家将不会有任何差别，都可以利用平台完成物流、资金流、商流、人才流、信息流
* 重点转向服务供为主的的集成供应链
* 区块链支持，目前公司已经帮助客户在生产环境中使用联盟链HyperLedger Fabric九个月时间，技术，运维已经成熟，这样可以让多个商家，金融机构参与集成供应链服务
* 实时化：目前使用golang开发的组件已经用户各个项目中，后面不管报表，消息，数据大屏，协同都是基于双向实时通信
* 国产基础设施：目前支持MYSQL和PGSQL，南大通用GBASE 8S已经在测试中，使用TiDB来管理感知层数据
* 工作流工具引入：支持创建灵活的工作流在适配各个用户
* 支持分析工作台模式和事务工作台模式

## 联系我们

欢迎加微信，共同学习、分享和探讨新型供应链系统以及相关技术。
